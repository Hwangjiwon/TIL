...
**일반적으로 콜백함수와 클로저는 인수위치에서 함수명을 생략하고 직접 함수코드를 지정하여 사용하는 익명함수**
...
JS 객체들은 시스템 생성객체와 사용자 생성객체로 구분할 수 있다.  
시스템 생성객체는 자바스크립트를 사용할 때 이미 존재하는 객체,  
사용자 생성객체는 사용자가 생성하는 객체이다.  

시스템 생성객체의 예로는 BOM, DOM이 있다.

- DOM(Document Object Model)  
document와 같이 객체들과 웹 브라우저에 로딩된 HTML 문서정보를 나타내는 객체  
문서의 구조화된 표현. 웹페이지의 객체 지향 표현  
웹페이지를 JS로 제어하기 위한 객체 모델  
윈도우에 로드된 문서를 의미  

- BOM(Browser Object Model)  
window, location, history, screen 과 같이 웹브라우저 관련 정보들 나타내는 객체   
자바스크립트가 브라우저와 소통하기 위한 모델.  
웹 브라우저의 창이나 프레임을 추상화하여 프로그래밍적으로 제어할 수 있도록 제공하는 수단  
 1) window 객체  
 현재 실행되고 있는 웹 브라우저 윈도우를 window 객체로 나타내고, 실행상태 등 관련된 정보들을 window의 하위 객체들로 표현해서 제공한다.  
 2) location 객체  
 웹브라우저의 주소 표시 영역을 나타내는 객체.  
 3) history 객체
 현재 웹 브라우저의 히스토리 정보 제공  
...
## 1. AJAX
 ...
 비동기 JS와 XML.  
 서버와 통신하기위해 XMLHttpRequest 객체 사용하는 것. 
 *  특징
 -- 페이지 새로고침 없이 서버에 요청  
 -- 웹 페이지 전체를 다시 로딩하지 않고도, 웹 페이지 일부만을 갱신  
 -- 서버로 부터 데이터를 받고 작업 수행  


JS 는 DOM을 Handling하는 것!!
form 태그는 이전의 메모리를 다 파기해버려
form은 무조건 동기방식, DOM도 저장하지 못함  
DOM방식은 메모리를 남기는 거
그걸 이용하려면 JS 이용해야하는 것
DOM을 남기면서 
XHR 객체(브라우저에서 제공하는 것이라 브라우저 버전이 중요) 만들어서 Request몰래 보내는 것이 AJAX  즉, DOM이 유지되는 방식  
